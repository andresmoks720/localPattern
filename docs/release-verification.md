...
- **Sender defaults:** Frame Duration `2000ms`, Error Correction `H`, QR Size `400`, Chunk Size `Auto`, Redundancy `3x`.
...
| RV-02 | Firefox Android → Chrome desktop | Pixel 7 (Android 14), Firefox 127.0 | macOS 14.6, Chrome 126.0.6478.127 | `image.png` / 312 KB | Frame Duration `2500ms`, ECC `H`, Redundancy `3x` | **PASS** | Needed slower frame duration to stabilize desktop webcam decoding from phone display. |
...
| RV-04 | File type coverage (`.png`) | macOS 14.6, Chrome 126.0.6478.127 | Pixel 7 (Android 14), Chrome 126.0.6478.103 | `diagram.png` / 1.2 MB | Frame Duration `2500ms`, QR Size `500`, Redundancy `3x` | **PASS** | Larger QR size improved decode consistency. |
...
| KL-02 | Older iOS devices (A12-era camera sensors) on dense packets | **Higher packet miss rate** | Keep chunk size on `Auto`, keep redundancy at `3x` or increase to `4x`, avoid background app switching during scan. |
| KL-03 | Long transfers with thermal throttling on mid-range Android | **Potential slowdown / frame drops** | Lower screen brightness slightly, keep device charging if possible, split files >5MB into smaller parts. |
...
- [ ] Re-run this verification matrix (RV-01 through RV-08) on release candidate build before creating a git tag.
- [ ] Confirm any failed scenario notes are either mitigated in docs or accepted in release notes.
- [ ] Attach updated timings for the 1MB stress run if defaults changed.
