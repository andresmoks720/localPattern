...
- Freeze redundancy for MVPv2 as fixed sender-side policy (prefer constant 1) and keep it non-user-tunable.
- Add a small “out-of-scope” guard doc and test notes.

## Phase 1 — Protocol byte-contract correctness (blocker)
1. Rework frame wire layouts to exact spec (no version byte, DATA payloadLen field, HEADER headerCrc32 field).
2. Lock CRC behavior to CRC-32/IEEE 802.3 and enforce packet CRC coverage `transferId + packetIndex + payload`.
...
1. Use non-empty success rule without END dependency; keep END mandatory for zero-byte completion and timeout signaling.
...
1. Add explicit preflight validation before READY (including QR encodability of all required frame types and HEADER CRC correctness).
...
> Note: `mvp.md` is the normative specification. This roadmap is implementation-order guidance only.
